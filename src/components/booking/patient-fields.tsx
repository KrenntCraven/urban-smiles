...
/**
 * Shared identity, contact, coverage, consent, and ID/HMO uploads.
 * Used by /book, the home quick-booking widget, and the service booking form
 * so all three submit the same fields to submitAppointment.
 */
...
export function PatientVerificationFields({
  ...
  const panelProps = (panel: Exclude<VerificationSection, "all" | "none">) => {
  ...
  const { onBlur: phoneOnBlur, onChange: phoneOnChange, ...phoneRest } = phone;
  const { onBlur: emailOnBlur, onChange: emailOnChange, ...emailRest } = email;
